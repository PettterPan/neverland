 ...
 * @author siwei.pan
 * @Description: ${todo}
 * @DATE 2019/10/15 3:46 PM
 *
 * 给定一个数组，将数组向右旋转k步，其中k为非负数。例如：
 *
 *
 *
...
/*新建一个大小和nums一样的数组，然后将旋转后的元素放入新的数组中，
  再遍历新数组赋值给nums。新数组的元素从nums的长度先减去k再加上i开始，
  直到i不小于k为止，这些元素是需要拿到数组前面去的。当i大于等于k时，新数组的元素要从nums的第1位开始，也就是将原数组前面的元素放到后面来。对于k的值，先判断对数组长度取余是否等于0，不等于0后才会开始下一步操作。
