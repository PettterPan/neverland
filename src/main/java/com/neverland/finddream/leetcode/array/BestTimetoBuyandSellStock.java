 ...
 * 如果只被允许完成最多一笔交易（即买入并卖出一股股票），
 * 请设计算法以找到最大利润。请注意，在购买之前不能出售股票。例如：
 *
 * 输入：[7,1,5,3,6,4]
 *
 * 输出：5
...
public class BestTimetoBuyandSellStock {
    ...
    /*
      ...
      正常情况：要想股票获利，需要满足低买高卖的条件，否则不获利甚至亏损。先将第1天的价格单独拿出来，
      接着开始遍历数组（从第二个元素开始遍历），如果第二天的价格高于第一天的价格，
      此时的最大利润是第二天的价格减去第一天的价格与最大利润初始值之间的最大值，
      否则买入价格就应该换成第二天的价格，依次向后循环，直到比较完所有的价格。
      最后，如果最大利润大于0，此时最大利润就是其本身，否则最大利润为0。
      */
